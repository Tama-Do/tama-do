...
let downloadTemplateApp = exports.downloadTemplateApp = (() => {
  var _ref3 = _asyncToGenerator(function* (templateId, selectedDir, opts) {
    ...
    try {
      yield (_joi || _load_joi()).default.promise.validate({ name: opts.name }, schema);
    } catch (e) {
    ...
    // Download files
    yield (_mkdirp || _load_mkdirp()).default.promise(root);
...
let extractTemplateApp = exports.extractTemplateApp = (() => {
  var _ref4 = _asyncToGenerator(function* (starterAppPath, name, root) {
    ...
    let author = yield (_UserSettings || _load_UserSettings()).default.getAsync('email', null);
    let packageJsonFile = new (_jsonFile || _load_jsonFile()).default(_path.default.join(root, 'package.json'));
    let packageJson = yield packageJsonFile.readAsync();
    ...
    yield packageJsonFile.writeAsync(data);

    // Custom code for replacing __NAME__ in main.js
    let mainJs = yield _fs.default.readFile.promise(_path.default.join(root, 'main.js'), 'utf8');
    let customMainJs = mainJs.replace(/__NAME__/g, data.name);
    yield _fs.default.writeFile.promise(_path.default.join(root, 'main.js'), customMainJs, 'utf8');

    // Update exp.json
    let expJson = yield _fs.default.readFile.promise(_path.default.join(root, 'exp.json'), 'utf8');
    let customExpJson = expJson.replace(/\"My New Project\"/, `"${data.name}"`).replace(/\"my-new-project\"/, `"${data.name}"`);
    yield _fs.default.writeFile.promise(_path.default.join(root, 'exp.json'), customExpJson, 'utf8');
  ...
  var _ref5 = _asyncToGenerator(function* (root) {
    ...
    if (!insideGit) {
      try {
        ...
        (_Logger || _load_Logger()).default.global.warn(`Unable to initialize git repo: ${e.stderr}`);
      }
    }
  });
