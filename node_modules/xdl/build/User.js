...
class UserManagerInstance {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (!user) {
        if (yield _this3.getLegacyUserData()) {
          throw new (_XDLError || _load_XDLError()).default((_ErrorCode || _load_ErrorCode()).default.LEGACY_ACCOUNT_ERROR, `We've updated our account system! Please login again by running \`exp login\`. Sorry for the inconvenience!`);
        }
  ...
  logoutAsync() {
    ...
    return _asyncToGenerator(function* () {
      ...
      // Logout of Intercom
      (_Intercom || _load_Intercom()).update(null, null);
    })();
  }
    ...
    return _asyncToGenerator(function* () {
      ...
      if ((!_this8._currentUser || _this8._currentUser.userId !== user.userId) && user.username && user.username !== '') {
        ...
        if (user.intercomUserHash) {
          (_Intercom || _load_Intercom()).update(user.username, user.intercomUserHash);
        }
      } else {
        (_Intercom || _load_Intercom()).update(null, null);
      }
      ...
      if (loginOptions.connection === 'Username-Password-Authentication') {
        try {
          ...
          if (err.message === 'invalid_user_password') {
            throw new (_XDLError || _load_XDLError()).default((_ErrorCode || _load_ErrorCode()).default.INVALID_USERNAME_PASSWORD, 'Invalid username or password');
          } else {
            throw err;
          }
        }
...
function _nodeAuth0InstanceWithOptions(options = {}) {
  ...
  if (auth0Options.management === true) {
    ...
    const ManagementClient = require('@exponent/node-auth0').ManagementClient;
    Auth0Instance = new ManagementClient(auth0Options);
  } else {
    const AuthenticationClient = require('@exponent/node-auth0').AuthenticationClient;
